...
:Description:
    ...
    The element load vector :code:`fe` can also be computed if a uniformly distributed transverse load is applied to the element. The optional input variable
        
    :code:`eq`:math:`= [q_{\bar{y}}]`

    then contains the distributed transverse load per unit length, :math:`q_{\bar{y}}`. Note that :code:`eq` is a scalar and not a vector as in :code:`beam2e`.
