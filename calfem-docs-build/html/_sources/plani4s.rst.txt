...
:Description:
    ...
    The input variables :math:`\mathbf{ex}`, :math:`\mathbf{ey}`, :math:`\mathbf{ep}` and the matrix :math:`\mathbf{D}` are defined in ``plani4e``.
    ...
    The index :math:`n` denotes the number of integration points used within the element, cf. ``plani4e``.
    The number of columns in ``es`` and ``et`` follows the size of ``D``.
...
:Theory:
    ...
    where the matrices :math:`\mathbf{D}`, :math:`\mathbf{B}^e`, and :math:`\mathbf{a}^e` are described in ``plani4e``, and where the integration points are chosen as evaluation points.
