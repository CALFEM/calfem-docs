...
:Theory:
    ...
    where :math:`\mathbf{G}` is described in :code:`beam2ge` and the transpose of :math:`\mathbf{a}^e` is stored in :code:`ed`. The displacements associated with bar action and beam action are determined as
    ...
    where :math:`L` is defined in :code:`beam2gxe` and
