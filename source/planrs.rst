...
:Description:
    ...
    The input variables :math:`\mathbf{ex}`, :math:`\mathbf{ey}`, :math:`\mathbf{ep}` and :math:`\mathbf{D}` are defined in ``planre``. The vector :math:`\mathbf{ed}` contains the nodal displacements :math:`\mathbf{a}^e` of the element and is obtained by the function ``extract`` as
...
:Theory:
    ...
    where the matrices :math:`\mathbf{D}`, :math:`\mathbf{B}^e`, and :math:`\mathbf{a}^e` are described in ``planre``, and where the evaluation point :math:`(x, y)` is chosen to be at the center of the element.
